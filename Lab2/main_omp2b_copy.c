...
int main (int argc, char **argv)
{
    ...
    double *b = (double *) calloc (i_size * j_size, sizeof (double));
    int i, j;
    ...
    */
    ...
    #pragma omp parallel for shared (a, b) collapse (2) num_threads (num_threads) schedule (static, chunk_size)
    for (int i = 3; i < i_size; i++)
    {
        for (int j = 0; j < j_size - 2; j++)
        {
            a[(i - 3) * j_size + j + 2] = sin (0.1 * b[i * j_size + j]);
        }
    }
